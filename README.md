...
- `.platform/kubernetes/**` — PlatformApplication CRD + dev/stg/prd kustomize overlays
  ...
  resource
- `Dockerfile`, `.platform/docker/{local,prd}/Dockerfile`, `Tiltfile` — container builds +
  local-dev orchestration (idiomatic starting points; adapt to your build)
- `.editorconfig`, `.gitattributes`, `.gitignore`

It generates **no** project scaffolding and **no** domain code — no `go.mod`, no `cmd/`,
no `internal/`, no `Makefile`. Everything renders **in place** at the destination root.
...
contract. This is the empty-tier sibling of `golang-rest-service-archetype` (full) and
`golang-service-basic-archetype` (minimal complete service).
